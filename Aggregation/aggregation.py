...
class Cockroach(Agent):
    ...
    def change_position(self):
        ...
        leave = (1-self.mean_neighbours/self.config.circle_area*100) ** (num_n*1.3**min(self.mean_neighbours, 0.0)) if num_n > 0 else 0.3
        ...
        if self.on_site():
            if not self.timer_j_started and not self.leaving:
